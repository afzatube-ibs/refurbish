<div class="manual-order-line" data-line-item data-match-status="{{ ($item['source_product_id'] ?? '') !== '' ? 'matched' : 'manual' }}">
    <div class="manual-order-line-top">
        <div class="manual-order-search-wrap">
            ...
            <div class="manual-order-search-row">
                <input type="search" class="df-input w-full" data-product-search placeholder="Search by name, LK model, IBS model, SM model…" autocomplete="off" value="{{ $item['product_name'] ?? '' }}">
    ...
    <div class="manual-order-line-grid">
        <div class="manual-order-line-span-2">
            ...
            <input type="text" name="items[{{ $index }}][product_name]" value="{{ $item['product_name'] ?? '' }}" class="df-input w-full" data-product-name required>
        ...
        <div>
            ...
            <input type="text" name="items[{{ $index }}][model]" value="{{ $item['model'] ?? '' }}" class="df-input w-full" data-model>
        </div>
        <div>
            <label class="manual-order-label">Option / variant</label>
            <input type="text" name="items[{{ $index }}][option]" value="{{ $item['option'] ?? '' }}" class="df-input w-full" data-option placeholder="Color: Blue">
        </div>
        <div>
            <label class="manual-order-label">Qty</label>
            <input type="number" name="items[{{ $index }}][quantity]" value="{{ $item['quantity'] ?? 1 }}" min="1" class="df-input w-full" data-qty required>
        ...
        <div>
            ...
            <input type="number" name="items[{{ $index }}][sale_price]" value="{{ $item['sale_price'] ?? '' }}" min="0" step="0.01" class="df-input w-full" data-sale-price required>
