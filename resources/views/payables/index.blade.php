...
@section('page-subtitle', auth()->user()->isAdmin() ? 'Supplier account balance and settlement entries' : 'Your payable summary (read-only)')
        ...
        <div>
            ...
            <select name="connection_id" id="connection_id"
                ...
                @foreach ($stores ?? [] as $store)
                    <option value="{{ $store->id }}" @selected(($selectedConnectionId ?? null) == $store->id)>{{ $store->store_url ?: 'Store #'.$store->id }}</option>
                @endforeach
            </select>
        </div>
        <div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    ...
    <div class="bg-white rounded-lg border border-slate-200 p-5">
        ...
        <p class="mt-1 text-xs text-slate-500">Store owner + supplier receipts</p>
    </div>
    ...
    <div class="lg:col-span-2">
        <div class="bg-white rounded-lg border border-slate-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-200 flex items-center justify-between gap-3">
                ...
                <a href="{{ route('reports.ledger', array_filter(['supplier_id' => $selectedSupplierId, 'connection_id' => $selectedConnectionId, 'from' => $from, 'to' => $to])) }}"
                   class="text-xs text-slate-500 hover:text-slate-800">Account statement →</a>
            </div>
